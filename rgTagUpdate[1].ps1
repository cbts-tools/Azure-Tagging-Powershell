...
ForEach($row in $userdata){ 
    [string]::Format("[Row {0} of {1}]  RG = {2}",([string] $CurrentRow),([string] $TotalRows),([string] $row.ResourceGroup) )
    $CurrentRow++
    # Pull Resource data to find resource ID
    $resource = Get-AzResourceGroup ([string] $row.ResourceGroup)
    Update-AzTag -ResourceId ([string] $resource.ResourceID) -Tag $tags -Operation Merge 
} 
